  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-gray-900">Reports - Charts</h1>
        <p className="text-sm text-gray-600">
          Visual summaries for shift cycles, merchants, and card spending.
        </p>
      </div>
          ...
          <Card className="p-4">
            ...
            <div className="mt-4 grid gap-3">
              {shiftSummaries.length === 0 && (
                <div className="text-sm text-gray-500">No shifts found in this range.</div>
              )}
              {shiftSummaries.slice(0, 8).map((shift) => (
                <div key={shift.id} className="rounded border p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <div className="font-medium">{shift.card}</div>
                      <div className="text-xs text-gray-500">{shift.employee} • {shift.shiftId}</div>
                    </div>
                    <div className="text-sm text-gray-700">
                      {shift.count} purchases • ${shift.total.toFixed(2)}
                    </div>
                  </div>
                  <div className="mt-2 h-2 rounded bg-gray-100">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, (shift.total / (shiftSummaries[0]?.total || 1)) * 100)}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          <Card className="p-4">
            ...
            <div className="mt-4 grid gap-3">
              {merchantTotals.length === 0 && (
                <div className="text-sm text-gray-500">No merchant data for this range.</div>
              )}
              {merchantTotals.slice(0, 10).map((merchant) => (
                <div key={merchant.merchant} className="rounded border p-3">
                  <div className="flex items-center justify-between">
                    <div className="font-medium">{merchant.merchant}</div>
                    <div className="text-sm text-gray-700">${merchant.total.toFixed(2)}</div>
                  </div>
          ...
          <Card className="p-4">
            ...
            <div className="mt-4 grid gap-3">
              {cardPeriodTotals.length === 0 && (
                <div className="text-sm text-gray-500">No card spend data for this range.</div>
              )}
              {cardPeriodTotals.slice(0, 10).map((row, index) => (
                <div key={`${row.period}-${index}`} className="rounded border p-3">
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-medium">{row.period} • {formatCardDisplay(row.cardId)}</div>
                    <div className="text-sm text-gray-700">${row.total.toFixed(2)}</div>
                  </div>
                  <div className="text-xs text-gray-500">{row.count} purchases</div>
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card className="p-4">
            ...
            <div className="mt-4 grid gap-3">
              {cardTypeDailyTotals.length === 0 && (
                <div className="text-sm text-gray-500">No card type data for this range.</div>
              )}
              {cardTypeDailyTotals.slice(0, 10).map((row, index) => (
                <div key={`${row.period}-${row.cardType}-${index}`} className="rounded border p-3">
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-medium">{row.period} • {row.cardType}</div>
                    <div className="text-sm text-gray-700">${row.total.toFixed(2)}</div>
                  </div>
                  <div className="text-xs text-gray-500">{row.count} purchases</div>
                </div>
